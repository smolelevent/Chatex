...
//OSZTÁLYON BELÜLI VÁLTOZÓK ELEJE -----------------------------------------------------------------
  ...
  final TextEditingController _searchController = TextEditingController(); //keresési szöveg
  final FocusNode _searchFocusNode = FocusNode(); //keresési mező fokuszálásához
  bool _isSearchFocused = false; //keresési mező fokuszálását elmentjük egy bool-ba
...
//HÁTTÉR FOLYAMATOK ELEJE -------------------------------------------------------------------------
  ...
  Future<void> _loadFriends() async {
    try {
      ...
      final response = await http.post(
        Uri.parse("http://10.0.2.2/ChatexProject/chatex_phps/chat/get/get_friend_list.php"),
      ...
      if (response.statusCode == 200) {
        setState(() {
          _friends = responseData; //ha sikeresen lekértük akkor a _friends tömb-be mentse a felhasználó barátait
          _isLoading = false;
        });
      } else {
      ...
      if (context.mounted) {
        final l10n = AppLocalizations.of(context)!;
        ToastMessages.showToastMessages(
          l10n.connectionErrorGettingFriendList,
          0.1,
          Colors.redAccent,
          Icons.error,
          Colors.black,
  ...
  void _startChatWith(int friendId) async {
    try {
      ...
      final response = await http.post(
        Uri.parse("http://10.0.2.2/ChatexProject/chatex_phps/chat/set/start_chat.php"),
        headers: {"Content-Type": "application/json"},
        body: jsonEncode({
          "sender_id": senderId, //a jelenleg bejelentkezett felhasználó
          "receiver_id": friendId, //az adott felhasználó id-je akivel chatelni szeretnénk
        }),
      );
      ...
      if (context.mounted) {
        final l10n = AppLocalizations.of(context)!;
        ToastMessages.showToastMessages(
          l10n.connectionErrorStartingChat,
          0.1,
          Colors.redAccent,
          Icons.error,
          Colors.black,
  ...
  Widget _buildSearchField() {
    return Container(
      ...
      child: FormBuilderTextField(
        ...
        focusNode: _searchFocusNode, //design változás (hintText és labelText) amikor fokuszban van
        onChanged: (searchQuery) {
          setState(() {
            _searchQuery = searchQuery ?? ""; //keresési mező tartalma
          });
        },
        keyboardType: TextInputType.name, //név kereséshez alkalmas billentyűzet kiosztás
  ...
  Widget _buildProfilePicture(String? receivedProfilePicture) {
    ...
    if (receivedProfilePicture != null && receivedProfilePicture.isNotEmpty) {
      if (receivedProfilePicture.startsWith("data:image/svg+xml;base64,")) {
        final svgString = utf8.decode(base64Decode(receivedProfilePicture.split(",")[1]));
