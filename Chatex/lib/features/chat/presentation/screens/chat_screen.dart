...
import 'package:chatex/features/chat/presentation/widgets/message_chat_bubble.dart';
import 'package:chatex/features/chat/presentation/widgets/file_chat_bubble.dart';
import 'package:chatex/features/chat/presentation/widgets/image_chat_bubble.dart';
import 'package:chatex/features/chat/presentation/screens/chat_information.dart';
import 'package:chatex/l10n/app_localizations.dart';
import 'package:chatex/core/local_storage/preferences.dart';
import 'package:chatex/core/constants/chat_constants.dart';
...
//OSZTÁLYON BELÜLI VÁLTOZÓK ELEJE -----------------------------------------------------------------
  ...
  final List<Map<String, dynamic>> _messages = <Map<String, dynamic>>[]; //üzenetek listája
...
//ÜZENET KÜLDŐ/KEZELŐ METÓDUSOK ELEJE -------------------------------------------------------------
  ...
  Future<void> _pickFiles() async {
    //TODO: pickFiles deprecated, kiterjesztések
    if (context.mounted) {
      final l10n = AppLocalizations.of(context)!;
      final picked = await FilePicker.pickFiles(
        dialogTitle: l10n.selectFiles,
        withData: true,
      ...
      if (oversizedFiles.isNotEmpty) {
        _showContentDialog(
          l10n.filesTooLargeTitle,
          l10n.filesTooLargeContent,
          actions: [
            TextButton(
  ...
  Future<void> _sendImageFromCamera() async {
    ...
    if (bytes.length > maxImageAttachmentSize) {
      ...
      if (context.mounted) {
        final l10n = AppLocalizations.of(context)!;
        _showContentDialog(
          l10n.imagesTooLargeTitle,
          l10n.imagesTooLargeContent,
          actions: [
            TextButton(
              onPressed: () => Navigator.pop(context),
              child: const Text("OK"),
...
//EGYÉB KIEGÉSZÍTŐ METÓDUSOK ELEJE ----------------------------------------------------------------
  ...
  void _showContentDialog(String informationTitle, String informationDetails, {List<Widget>? actions}) {
    showDialog(
      context: context,
  ...
  void _checkMessageLength() {
    ...
    if (_messageController.text.length > maxMessageLength) {
      _showContentDialog(
        l10n.messageTooLongTitle,
        l10n.messageTooLongContent(maxMessageLength),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
            child: const Text("OK"),
                ...
                children: [
                  ...
                  AutoSizeText(
                    ...
                    _currentStatus == "online" && _currentSignedIn == 1
                        ? l10n.online
                        : l10n.lastSeen(formatLastSeen(_currentLastSeen)),
                    style: const TextStyle(
                      fontSize: 14,
                      color: Colors.grey,
                    ),
      ...
      child: BottomAppBar(
        ...
        child: Row(
          children: [
            ...
            _buildIcon(
              color: Colors.deepPurpleAccent,
              tooltip: _showSendIcon ? l10n.sendMessage : l10n.emojiButton,
